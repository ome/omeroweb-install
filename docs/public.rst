...
#. Create a dedicated read-only group to host the raw data underlying the
   publication (see :omerodoc:`cli/usergroup`).
...
:omerodoc:`separate web server <unix/install-web/web-deployment>` for servicing
...
#. Move the original images into the dedicated group using OMERO.web or
   :omerodoc:`OMERO.cli <users/cli/chgrp>`. The CLI is best used where Images or
...
#. Having all the data belong to one user simplifies the UI experience for
   ...
   'Chown' privilege (see :omerodoc:`sysadmins/restricted-admins` and
   :omerodoc:`users/cli/chown`).
